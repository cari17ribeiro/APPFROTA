...
export default function ValidacaoExtrasScreen({ supabase, onLogout }) {
  ...
  const [fotoHistoricoModal, setFotoHistoricoModal] = useState(null);
  ...
  const carregarDashboard = async () => {
    ...
    try {
      ...
      const acertos = registros.filter(v => v.ocr_correto === true).length;
      const erros = registros.filter(v => v.ocr_correto === false).length;
  ...
  const aprovarViagem = async (e) => {
    ...
    const normalizarValor = (valor) => (valor || '').trim().toUpperCase();
    const dadosOriginais = {
      container: normalizarValor(viagemSelecionada.container),
      placa: normalizarValor(viagemSelecionada.placa),
      frota: normalizarValor(viagemSelecionada.frota),
      carreta: normalizarValor(viagemSelecionada.carreta)
    };
    const dadosEditados = {
      container: normalizarValor(editContainer),
      placa: normalizarValor(editPlaca),
      frota: normalizarValor(editFrota),
      carreta: normalizarValor(editCarreta)
    };
    const ocrAcertou =
      dadosOriginais.container !== '' &&
      dadosOriginais.placa !== '' &&
      dadosOriginais.container === dadosEditados.container &&
      dadosOriginais.placa === dadosEditados.placa &&
      dadosOriginais.frota === dadosEditados.frota &&
      dadosOriginais.carreta === dadosEditados.carreta;

    try {
      const { error } = await supabase
        .from('viagens_extra')
        .update({
          container: dadosEditados.container,
          placa: dadosEditados.placa,
          frota: dadosEditados.frota,
          carreta: dadosEditados.carreta,
          status: 'Validado',
          ocr_correto: ocrAcertou
        })
  ...
  const excluirViagem = async () => {
    ...
    const confirmar = window.confirm('Tem certeza que deseja excluir esta viagem extra? Essa ação não poderá ser desfeita.');
    if (!confirmar) return;
    ...
    try {
      ...
      setViagens((viagensAtuais) => viagensAtuais.filter((viagem) => viagem.id !== viagemSelecionada.id));
      fecharModal();
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col font-sans">
      <header className="bg-slate-900 text-white shadow-md">
        ...
        <div className="flex px-5 space-x-1 mt-2 overflow-x-auto">
          <button
            onClick={() => setActiveTab('validacao')}
            className={`px-6 py-3 font-bold text-sm rounded-t-xl transition-colors flex items-center whitespace-nowrap ${activeTab === 'validacao' ? 'bg-slate-100 text-slate-900' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'}`}
          >
      ...
      <main className="flex-1 p-6 max-w-7xl mx-auto w-full space-y-6">
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 flex flex-wrap gap-4 items-end">
          ...
          <div className="flex-1 min-w-[150px]">
            ...
            <input type="date" value={filtroDataInicio} onChange={e => setFiltroDataInicio(e.target.value)} className="w-full bg-slate-50 border-slate-200 rounded-xl p-2 text-sm outline-none border focus:ring-2 focus:ring-blue-500/20" />
          </div>

          <div className="flex-1 min-w-[150px]">
            <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Data Final</label>
            <input type="date" value={filtroDataFim} onChange={e => setFiltroDataFim(e.target.value)} className="w-full bg-slate-50 border-slate-200 rounded-xl p-2 text-sm outline-none border focus:ring-2 focus:ring-blue-500/20" />
          </div>

          <button
            onClick={activeTab === 'validacao' ? carregarViagensPendentes : activeTab === 'dashboard' ? carregarDashboard : carregarHistorico}
            className="bg-blue-50 text-blue-600 hover:bg-blue-100 px-6 py-2.5 rounded-xl font-bold text-sm transition-colors border border-blue-200 flex items-center"
          >
            <Filter className="w-4 h-4 mr-2" /> Filtrar
          </button>

          <button onClick={() => setIsExportExtraOpen(true)} className="bg-indigo-50 text-indigo-700 hover:bg-indigo-100 px-6 py-2.5 rounded-xl font-bold text-sm transition-colors border border-indigo-200 flex items-center ml-auto">
              ...
              <table className="w-full text-left text-sm whitespace-nowrap">
                ...
                <tbody className="divide-y divide-slate-100">
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan="6" className="p-8 text-center text-slate-500 font-medium">Nenhuma viagem extra pendente de validação.</td>
                    </tr>
                  ) : (
                    viagensFiltradas.map(viagem => {
                      ...
                      return (
                        <tr key={viagem.id} className="hover:bg-slate-50 transition-colors">
                          <td className="p-4 font-medium text-slate-700">{viagem?.data ? viagem.data.split('-').reverse().join('/') : '--/--/----'} às {viagem?.hora || '--:--'}</td>
                          <td className="p-4 font-bold text-slate-900">{viagem?.motorista || 'Desconhecido'}</td>
                          <td className="p-4 font-medium text-blue-600 bg-blue-50/50">{viagem.tipo_operacao}</td>
                          <td className="p-4 text-slate-600">{viagem.origem} <ArrowRight className="inline w-3 h-3 text-slate-400 mx-1" /> {viagem.destino}</td>
                          <td className="p-4">
                            <div className="flex flex-col gap-2 items-center justify-center">
                              {falhaOCR ? (
                                <span className="inline-flex items-center px-2 py-1 bg-amber-100 text-amber-700 rounded-md text-xs font-bold"><AlertCircle className="w-3 h-3 mr-1" /> Revisão Necessária</span>
                              ) : (
                                <span className="inline-flex items-center px-2 py-1 bg-emerald-100 text-emerald-700 rounded-md text-xs font-bold"><CheckCircle className="w-3 h-3 mr-1" /> IA Completa</span>
                              )}
                              {viagem.photo_source === 'galeria' && (
                                <button onClick={() => setJustificativaModal(viagem.justificativa || 'Nenhuma justificativa fornecida pelo motorista.')} className="inline-flex items-center px-2 py-1 bg-purple-100 hover:bg-purple-200 text-purple-700 rounded-md text-xs font-bold transition-colors cursor-pointer shadow-sm border border-purple-200" title="Ver Justificativa">
                          ...
                          <td className="p-4 text-right">
                            <button onClick={() => abrirModal(viagem)} className="bg-slate-900 hover:bg-slate-800 text-white px-4 py-2 rounded-lg font-bold transition-colors shadow-sm">Analisar</button>
                          </td>
                        </tr>
                      );
                    })
                ...
                <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row items-center justify-center gap-12">
                  <div className="relative w-56 h-56">
                    <svg viewBox="0 0 36 36" className="w-full h-full transform -rotate-90 drop-shadow-md">
                      ...
                      <circle cx="18" cy="18" r="15.91549430918954" fill="transparent" stroke="#10b981" strokeWidth="5" strokeDasharray={`${dashMetrics.taxa} ${100 - dashMetrics.taxa}`} strokeDashoffset="0" className="transition-all duration-1000 ease-out" />
                    </svg>
                  ...
                  <div className="flex flex-col gap-6">
                    <h3 className="text-lg font-black text-slate-800 flex items-center mb-2"><PieChart className="w-5 h-5 mr-2 text-indigo-500" /> Proporção de Desempenho</h3>
                  ...
                  {loadingHistorico ? (
                    <tr>
                      <td colSpan="8" className="p-8 text-center text-slate-500">
                        <Loader2 className="w-6 h-6 animate-spin mx-auto mb-2 text-blue-500" /> Carregando histórico...
                      </td>
                    </tr>
                  ) : historicoFiltrado.length === 0 ? (
                    <tr>
                      <td colSpan="8" className="p-8 text-center text-slate-500 font-medium">Nenhum registro encontrado no histórico.</td>
                    </tr>
                  ) : (
                    historicoFiltrado.map((viagem) => (
                      <tr key={viagem.id} className="hover:bg-slate-50 transition-colors">
                        <td className="p-4 font-medium text-slate-700">{viagem?.data ? viagem.data.split('-').reverse().join('/') : '--/--/----'} às {viagem?.hora || '--:--'}</td>
                        <td className="p-4 font-bold text-slate-900">{viagem?.motorista || 'Desconhecido'}</td>
                        <td className="p-4 font-medium text-blue-600 bg-blue-50/50">{viagem.tipo_operacao || '-'}</td>
                        ...
                        <td className="p-4 text-center">
                          {viagem.ocr_correto === true ? (
                            <span className="inline-flex items-center px-2 py-1 bg-emerald-100 text-emerald-700 rounded-md text-xs font-bold"><CheckCircle className="w-3 h-3 mr-1" /> Acerto</span>
                          ) : viagem.ocr_correto === false ? (
                            <span className="inline-flex items-center px-2 py-1 bg-rose-100 text-rose-700 rounded-md text-xs font-bold"><AlertCircle className="w-3 h-3 mr-1" /> Ajustado</span>
                          ) : (
                        ...
                        <td className="p-4 text-right sticky right-0 bg-white shadow-[-8px_0_12px_-12px_rgba(15,23,42,0.45)]">
                          {viagem.comprovante_url ? (
                            ...
                            <span className="text-slate-400 font-medium">Sem foto</span>
                          )}
                        </td>
                      </tr>
                    ))
      ...
      {justificativaModal && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md flex items-center justify-center z-[100] p-4" onClick={() => setJustificativaModal(null)}>
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl flex flex-col gap-4 animate-in zoom-in duration-200" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-slate-600 font-medium leading-relaxed bg-slate-50 p-4 rounded-xl border border-slate-100">{justificativaModal}</p>
            <button onClick={() => setJustificativaModal(null)} className="mt-2 w-full bg-slate-900 hover:bg-slate-800 text-white py-3 rounded-xl font-bold transition-colors shadow-md">Fechar</button>
          </div>
        </div>
      )}

      {fotoHistoricoModal && (
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-sm flex items-center justify-center z-[110] p-4" onClick={() => setFotoHistoricoModal(null)}>
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl max-h-[92vh] flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="bg-slate-50 border-b border-slate-200 px-6 py-4 flex justify-between items-center">
              <div>
                <h3 className="text-lg font-black text-slate-800">Foto Anexada</h3>
            ...
            <div className="bg-slate-900 flex-1 min-h-[420px] p-4 flex items-center justify-center overflow-auto">
              <a href={fotoHistoricoModal.comprovante_url} target="_blank" rel="noreferrer" className="w-full h-full flex items-center justify-center">
                <img src={fotoHistoricoModal.comprovante_url} alt="Comprovante anexado" className="max-w-full max-h-[75vh] object-contain rounded-2xl" />
              </a>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md flex items-center justify-center z-[100] p-4" onClick={() => setIsExportExtraOpen(false)}>
          <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl flex flex-col gap-6 animate-in zoom-in duration-200" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center border-b border-slate-100 pb-4">
              <h3 className="text-xl font-black text-slate-800 flex items-center"><FileSpreadsheet className="w-6 h-6 mr-2 text-indigo-600" /> Exportar Extras</h3>
              <button onClick={() => setIsExportExtraOpen(false)} className="text-slate-400 hover:text-rose-500 transition-colors"><XCircle className="w-6 h-6" /></button>
            </div>
            ...
            <div className="pt-2 flex gap-3">
              <button onClick={() => setIsExportExtraOpen(false)} className="flex-1 bg-white border border-slate-200 hover:bg-slate-50 text-slate-600 py-3 rounded-xl font-bold transition-colors">Cancelar</button>
        ...
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-5xl flex flex-col overflow-hidden max-h-[90vh]">
            <div className="bg-slate-50 border-b border-slate-200 px-6 py-4 flex justify-between items-center">
              ...
              <button type="button" onClick={fecharModal} disabled={isSaving} className="text-slate-400 hover:text-slate-600 disabled:opacity-50"><XCircle className="w-6 h-6" /></button>
            </div>
                ...
                <div className="bg-slate-50 p-6 rounded-3xl border border-slate-200 h-fit space-y-5">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="col-span-2 sm:col-span-1">
                      ...
                      <input type="text" value={editPlaca} onChange={(e) => setEditPlaca(e.target.value.toUpperCase())} className={`w-full bg-white rounded-xl p-3 text-sm font-bold uppercase outline-none border-2 focus:border-blue-500 ${!editPlaca ? 'border-amber-400 bg-amber-50' : 'border-slate-200'}`} placeholder="ABC1D23" />
                    </div>
                    <div className="col-span-2 sm:col-span-1">
                      <label className="block text-xs font-bold text-slate-500 uppercase mb-1.5">Frota</label>
                      <input type="text" value={editFrota} onChange={(e) => setEditFrota(e.target.value.toUpperCase())} className="w-full bg-white border-slate-200 border-2 rounded-xl p-3 text-sm font-bold uppercase outline-none focus:border-blue-500" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase mb-1.5">Carreta</label>
                    <input type="text" value={editCarreta} onChange={(e) => setEditCarreta(e.target.value.toUpperCase())} className="w-full bg-white border-slate-200 border-2 rounded-xl p-3 text-sm font-bold uppercase outline-none focus:border-blue-500" />
                  </div>
            ...
            <div className="bg-white border-t border-slate-200 px-6 py-4 flex flex-col sm:flex-row justify-between gap-3">
              <button type="button" onClick={excluirViagem} disabled={isSaving} className="bg-rose-50 hover:bg-rose-100 text-rose-600 border border-rose-200 px-6 py-3 rounded-xl font-bold flex items-center justify-center transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                <Trash2 className="w-5 h-5 mr-2" /> Excluir Viagem
              </button>
              <div className="flex flex-col sm:flex-row gap-3">
                <button type="button" onClick={fecharModal} disabled={isSaving} className="px-6 py-3 text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-xl font-bold transition-colors disabled:opacity-50">Cancelar</button>
                <button type="submit" form="form-validacao" disabled={isSaving || !editContainer || !editPlaca} className="bg-emerald-500 hover:bg-emerald-600 text-white px-8 py-3 rounded-xl font-bold flex items-center justify-center transition-colors shadow-md disabled:opacity-50 disabled:cursor-not-allowed">
                  {isSaving ? (<><Loader2 className="w-5 h-5 animate-spin mr-2" />Processando...</>) : (<><Save className="w-5 h-5 mr-2" />Aprovar Viagem</>)}
                </button>
              </div>
            </div>
          </div>
